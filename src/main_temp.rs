...
fn compile_java(train_src_dir: &str, validation_src_dir: &str, test_src_dir: &str, output_dir: &str) {
    ...
    // src_dir to output mapping
    let dirs = [
        (train_src_dir, &train_output),
        (validation_src_dir, &validation_output),
        (test_src_dir, &test_output),
    ];

    // run javac for each file separately
    for (src_dir, output_path) in dirs {
...
fn main() -> std::io::Result<()> {
    let paths = (
        "dataset/train/java",                // original train .java files
        "dataset/val/java",           // original validation .java files
        "dataset/test/java",                 // original test .java files
